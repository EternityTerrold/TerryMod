...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (songLowercase)
		{
			...
			case 'hard-rut':
				dialogue = CoolUtil.coolTextFile(Paths.txt('hard-rut/hard-rutdialogue'));
			case 'spacetail':
				dialogue = CoolUtil.coolTextFile(Paths.txt('spacetail/spacetaildialogue'));
			case 'ram-error':
				dialogue = CoolUtil.coolTextFile(Paths.txt('ram-error/ram-errordialogue'));
			case 'bopeebo':
				dialogue = [
					'HEY!',
			...
			case 'rutbg':
				{
						...
						rutbg = new FlxSprite(-600, -200);
						rutbg.frames = Paths.getSparrowAtlas('club1/rutbg');
						rutbg.animation.addByPrefix('rutbg', "rutbg", 24, true);
						rutbg.antialiasing = true;
						//rutbg.scrollFactor.set(0.3, 0.3);
						if(FlxG.save.data.distractions){
							add(rutbg);
						}
				}
			...
			case 'moonstage':
					{
						...
						var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('club2/moonstage'));
						bg.antialiasing = true;
			...
			case 'week3bg':
					{
						...
						var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('club3/week3bg'));
						bg.antialiasing = true;
						bg.scrollFactor.set(0.9, 0.9);
						bg.active = false;
						add(bg);

						arrow = new Character(450, 100, 'arrow');
						arrow.debugMode = true;
		...
		if (generatedMusic)
			{
				...
				if (dad.animation.curAnim.name == 'idle' && curStage == 'week3bg') {
				...
				{	
					...
					if (!daNote.mustPress && daNote.wasGoodHit)
					{
						...
						if (curStage == 'week3bg') {
							...
							{
								case 2:
									arrow.playAnim('singUP', true);
								case 3:
									arrow.playAnim('singRIGHT', true);
								case 1:
									arrow.playAnim('singDOWN', true);
								case 0:
									arrow.playAnim('singLEFT', true);
							}
						}
	...
	*/
	...
	override function beatHit()
	{
		...
		switch (curStage)
		{
			...
			case 'rutbg':
				if(FlxG.save.data.distractions){
					rutbg.animation.play('rutbg',true);
				}
